...
use key_server_cluster::cluster_sessions::{ClusterSession, ClusterSessions, ClusterSessionsContainer, create_cluster_view};
...
pub trait MessageProcessor: Send + Sync {
	...
	fn try_continue_session(&self, session: Option<Arc<KeyVersionNegotiationSession<KeyVersionNegotiationSessionTransport>>>);
...
impl SessionsMessageProcessor {
			...
			Err(error) => {
				...
				if !message.is_error_message() {
					let session_id = message.into_session_id()
						.expect("session_id only fails for cluster messages; only session messages are passed to process_message; qed");
					let session_nonce = message.session_nonce()
						.expect("session_nonce only fails for cluster messages; only session messages are passed to process_message; qed");
			...
			match message_result {
				Ok(_) => {
					...
					if session.is_finished() {
						info!(target: "secretstore_net", "{}: {} session completed", self.self_key_pair.public(), S::type_name());
			...
			Message: IntoSessionId<S::Id>
	{
		...
		// get or create new session, if required
		let session_id = message.into_session_id()
			.expect("into_session_id fails for cluster messages only; only session messages are passed to prepare_session; qed");
		...
		match is_initialization_message || is_delegation_message {
			...
			true => {
				...
				let master = if is_initialization_message { sender.clone() } else { self.self_key_pair.public().clone() };
	...
	fn process_cluster_message(&self, connection: Arc<Connection>, message: ClusterMessage) {
		match message {
			ClusterMessage::KeepAlive(_) => connection.send_message(Message::Cluster(ClusterMessage::KeepAliveResponse(message::KeepAliveResponse {
				session_id: None,
			}))),
...
impl MessageProcessor for SessionsMessageProcessor {
	...
	fn process_connection_message(&self, connection: Arc<Connection>, message: Message) {
		...
		match message {
			Message::Generation(message) => self.process_message(&self.sessions.generation_sessions, connection, Message::Generation(message))
				.map(|_| ()).unwrap_or_default(),
			Message::Encryption(message) => self.process_message(&self.sessions.encryption_sessions, connection, Message::Encryption(message))
				.map(|_| ()).unwrap_or_default(),
			Message::Decryption(message) => self.process_message(&self.sessions.decryption_sessions, connection, Message::Decryption(message))
				.map(|_| ()).unwrap_or_default(),
			Message::SchnorrSigning(message) => self.process_message(&self.sessions.schnorr_signing_sessions, connection, Message::SchnorrSigning(message))
				.map(|_| ()).unwrap_or_default(),
			Message::EcdsaSigning(message) => self.process_message(&self.sessions.ecdsa_signing_sessions, connection, Message::EcdsaSigning(message))
				.map(|_| ()).unwrap_or_default(),
			Message::ServersSetChange(message) => {
				...
				if is_initialization_message {
					if let Some(session) = session {
						self.servers_set_change_creator_connector.set_key_servers_set_change_session(session.clone());
					}
				}
			},
			Message::KeyVersionNegotiation(message) => {
				let session = self.process_message( &self.sessions.negotiation_sessions, connection, Message::KeyVersionNegotiation(message));
				self.try_continue_session(session);
			},
			Message::ShareAdd(message) => self.process_message(&self.sessions.admin_sessions, connection, Message::ShareAdd(message))
				.map(|_| ()).unwrap_or_default(),
