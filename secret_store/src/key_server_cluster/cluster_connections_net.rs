...
use key_server_cluster::net::{accept_connection as io_accept_connection, connect as io_connect, Connection as IoConnection};
...
impl NetConnectionsData {
	...
	pub fn for_each_connection<F: Fn(&NetConnection)>(&self, f: F) {
		self.container.read().connections.values()
			.for_each(|connection| f(&**connection))
	}

	/// Executes closure for each disconnected node.
	pub fn for_each_disconnected_address<F: Fn(&SocketAddr)>(&self, f: F) {
		let container = self.container.read();
		container.nodes.iter()
			.filter(|(node_id, _)| !container.connections.contains_key(node_id))
			.for_each(|(_, addr)| f(addr))
	}
	...
	pub fn insert(&self, connection: Arc<NetConnection>) -> bool {
		...
		trace!(target: "secretstore_net", "{}: inserting connection to {} at {}. Connected to {} of {} nodes",
			self.self_key_pair.public(), node, connection.node_address(), container.connections.len() + 1, container.nodes.len());
		...
		.then(move |result|
			match result {
				Ok((_, Ok(message))) => {
					...
					let process_messages_future = net_process_connection_messages(data.clone(), connection).then(|_| Ok(()));
...
fn net_keep_alive(data: Arc<NetConnectionsData>) {
	let now = Instant::now();
	data.for_each_connection(|connection| {
...
fn net_connect_disconnected(data: Arc<NetConnectionsData>) {
	data.for_each_disconnected_address(|address| net_connect(data.clone(), address.clone()));
}
