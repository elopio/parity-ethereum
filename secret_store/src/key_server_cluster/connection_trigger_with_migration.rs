...
use key_server_cluster::cluster::{ServersSetChangeParams};
...
impl TriggerSession {
	...
	/// Maintain session.
	pub fn maintain(&mut self, action: SessionAction, server_set: &KeyServerSetSnapshot) -> Option<ServersSetChangeParams> {
		...
		match signatures {
			...
			Err(err) => {
				trace!(target: "secretstore_net", "{}: failed to sign servers set for auto-migrate session with: {}",
